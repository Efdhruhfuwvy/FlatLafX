	...
	extends FlatWindowAbstractIcon
{
	...
	protected void paintIconAt1x( Graphics2D g, int x, int y, int width, int height, double scaleFactor ) {
		if( SystemInfo.isGNOME ) {
			...
			Path2D r1 = FlatUIUtils.createRectangle( ix + ro2, iy, rwh, rwh,
				thickness );

			// lower-left rectangle
			Path2D r2 = FlatUIUtils.createRectangle( ix, iy + ro2, rwh, rwh,
				thickness );

			// paint upper-right rectangle
			Area area = new Area( r1 );
			area.subtract( new Area(
				new Rectangle2D.Float( ix + ro2 - thickness, iy + thickness,
					rwh, rwh ) ) );
			g.fill( area );
